...
Industries <- c("All industries [T001]",                                                                                      
...
for (indLG in Industries) {
  ...
  (RMS03 <- RMS(r3$REV03))
  (RMS04 <- RMS(r3$REV04))
  Stats[idx,1] <- round(MEAN01,3)
  Stats[idx,2] <- round(MEAN02,3)
  Stats[idx,3] <- round(MEAN03,3)
  Stats[idx,4] <- round(MEAN04,3)
  Stats[idx,5] <- round(MABS01,3)
  Stats[idx,6] <- round(MABS02,3)
  Stats[idx,7] <- round(MABS03,3)
  Stats[idx,8] <- round(MABS04,3)
  Stats[idx,9] <-  round(RMS01,3)
  Stats[idx,10] <- round(RMS02,3)
  Stats[idx,11] <- round(RMS03,3)
  Stats[idx,12] <- round(RMS04,3)
  Stats[idx,13] <- indLG
  p0 <- ggplot(r3)+
...
dev.off()
colnames(Stats) <- c("MEAN01","MEAN02","MEAN03","MEAN04","MABS01","MABS02",
  "MABS03","MABS04","RMS01","RMS02","RMS03","RMS04","NAICS")
WriteXLS(Stats,"GDP_rev_stats.xls")
